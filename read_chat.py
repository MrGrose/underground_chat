...
def create_parser():
    parser = configargparse.ArgParser(default_config_files=["config_read.ini"], description="Загрузка истории переписки из чата")
    parser.add("-c", "--config", is_config_file=True, help="Путь к конфиг. файлу")
    parser.add("--host", env_var="HOST", help="host сервера")
    parser.add("--port", env_var="READ_PORT", help="port сервера")
    parser.add("--history", env_var="HISTORY", help="Путь к файлу с историей переписки")
